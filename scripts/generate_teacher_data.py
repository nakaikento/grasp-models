...
使い方:
    # 基本実行（AI Hub全データ）
    python generate_teacher_data.py --input data/aihub --output output/teacher_data.jsonl

    # サンプル数指定
    python generate_teacher_data.py --input data/aihub --output output/teacher_data.jsonl --limit 1000000

    # 再開（前回の続きから）
    python generate_teacher_data.py --input data/aihub --output output/teacher_data.jsonl --resume

    # バッチサイズ調整（VRAM不足時）
    python generate_teacher_data.py --input data/aihub --output output/teacher_data.jsonl --batch-size 32
...
推定処理時間:
    - RTX A6000: ~7 samples/s → 100万件 ≈ 40時間
    - 複数GPU並列で短縮可能
"""
...
def main():
    parser = argparse.ArgumentParser(description="教師データ大規模生成")
